...
local function UpdateTracers()
    for plr, tracer in pairs(TracerObjects) do
        ...
        if root and myRoot then
            local screenPos = workspace.CurrentCamera:WorldToViewportPoint(root.Position)
...
PlayerTab:CreateToggle({Name = "God Mode", Callback = function(s)
    local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
    if hum then
        hum.MaxHealth = s and 1e6 or 100
        hum.Health = s and 1e6 or 100
    end
end})
...
Rayfield:Notify({Title = "MM2 Hacks", Content = "Menu Loaded Successfully!", Duration = 6})
